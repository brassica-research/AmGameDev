...
static func hit_probability(range_yards: float, smoke: float, cohesion: float,
		drill_level: int, hold_bonus: float = 0.0) -> float:
...
## Resolve one volley: `shooters` muskets firing at a target of
## `target_count` effectives. Returns casualties inflicted.
static func resolve(shooters: int, range_yards: float, smoke: float,
		cohesion: float, drill_level: int, hold_bonus: float,
		rng: RandomNumberGenerator) -> int:
	var p := hit_probability(range_yards, smoke, cohesion, drill_level, hold_bonus)
	var hits := 0
	for i in shooters:
		if rng.randf() < p:
			hits += 1
